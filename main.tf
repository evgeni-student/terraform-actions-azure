...
resource "azurerm_service_plan" "task-board-plan" {
...
resource "azurerm_linux_web_app" "linux-web-app-server" {
  name                = "${var.app_service_name}-${random_integer.ri.result}"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  service_plan_id     = azurerm_service_plan.task-board-plan.id
...
resource "azurerm_app_service_source_control" "git" {
  app_id   = azurerm_linux_web_app.linux-web-app-server.id
